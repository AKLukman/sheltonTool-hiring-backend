...
app.get("/categorized-products", async (req, res) => {
  ...
  const PAGE_SIZE = 5;
  try {
    const distinctCategories = await userCollection
      .aggregate([
        { $group: { _id: "$category" } },
    ...
    for (const { category } of distinctCategories) {
      const products = await userCollection
        .find({ category })
        .limit(PAGE_SIZE)
        .toArray();
...
app.delete("/deleteProduct/:productId", async (req, res) => {
  ...
  try {
    const result = await placedProducts.deleteOne({
...
app.post("/add-review/:toolId", async (req, res) => {
  try {
    const { toolId } = req.params;
    const { repliedCommentId, reviewerName, reviewerComment, reviewTime } =
      req.body;
    const tool = await userCollection.findOneAndUpdate(
      {
        _id: new ObjectId(toolId),
